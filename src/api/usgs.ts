...
const cqlString = (value: string) => value.replace(/'/g, "''");

const searchVariants = (term: string) =>
  [...new Set([term, term.toLowerCase(), titleCase(term)])]
    .filter(Boolean)
    .map((variant) => `monitoring_location_name LIKE '%${cqlString(variant)}%'`);

const stationNameFilter = (term: string) => {
...
export const searchStations = async (query: string): Promise<Station[]> => {
  ...
  const filter =
    terms.length > 0
      ? terms.map(stationNameFilter).join(" AND ")
      : stationNameFilter(trimmed);
